<template>
  <section
    id="team-section"
    class="relative z-10 w-full bg-black overflow-hidden"
  >
    <!-- The GSAP Horizontal Scroll Wrapper -->
    <div ref="horizontalWrapper" class="flex h-[100svh] w-full flex-nowrap will-change-transform relative">
      
      <!-- Navigation Buttons (Fixed during pin) -->
      <div class="absolute top-6 left-6 md:top-12 md:left-12 z-50 flex gap-4 pointer-events-auto">
        <button @click="goHome" class="w-12 h-12 md:w-14 md:h-14 rounded-full bg-flax-smoke-500 text-black flex items-center justify-center hover:bg-white hover:scale-110 transition-all duration-300 shadow-[0_4px_25px_rgba(0,0,0,0.5)] border-2 border-black/10" title="Go to Home">
        ...
        <button @click="goExit" class="w-12 h-12 md:w-14 md:h-14 rounded-full bg-flax-smoke-500 text-black flex items-center justify-center hover:bg-white hover:scale-110 transition-all duration-300 shadow-[0_4px_25px_rgba(0,0,0,0.5)] border-2 border-black/10" title="Exit Section">
      ...
      <!-- INTRO PANEL (Panel 1) -->
      <div class="horizontal-panel shrink-0 w-screen h-[100svh] flex flex-col items-center justify-center bg-[#202623] relative border-r border-white/10 px-4">
        <!-- Background Decor -->
        <div class="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-flax-smoke-500/20 via-transparent to-transparent"></div>
        
        <h3 class="text-6xl md:text-[8rem] leading-none font-black text-flax-smoke-50 uppercase tracking-tighter mb-12 drop-shadow-2xl z-10 text-center">
          OUR TEAM
        </h3>
        
        <!-- Year Selector -->
        <div class="flex flex-wrap items-center justify-center gap-3 md:gap-5 max-w-5xl z-10">
          <button
            v-for="year in years"
            :key="year"
            @click="changeYear(year)"
            class="px-6 py-3 rounded-full text-sm md:text-lg font-black transition-all duration-500 tracking-widest shadow-2xl border"
            :class="
              selectedYear === year
        ...
        <div v-if="groupedTeam.length === 0" class="mt-12 text-flax-smoke-600 text-xl italic z-10">
          Team data for {{ selectedYear }} is currently being updated.
        </div>
      </div>

      <!-- TEAM GROUPS (Panels 2+) -->
      <div 
        v-for="(group, gIdx) in groupedTeam" 
        :key="group.title"
        class="horizontal-panel shrink-0 w-screen h-[100svh] flex relative bg-[#202623] md:py-12 lg:py-16 md:px-12 lg:px-24"
      >

        <!-- Framed Expanding Accordions Container (Full on mobile, rounded box on laptop) -->
        <div class="flex flex-col md:flex-row h-full w-full md:rounded-[2.5rem] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border-0 md:border-[6px]" :class="getGroupTheme(gIdx).borderColor">
          <div 
            v-for="member in group.members" 
            :key="member.id"
            class="relative flex-1 h-full w-full group overflow-hidden transition-[flex-grow] duration-500 ease-out hover:flex-[2] md:hover:flex-[1.5]"
          >
          <!-- Massive Image — object-contain keeps full face visible -->
          <div class="absolute inset-0 w-full h-full bg-[#111] flex items-center justify-center overflow-hidden">
            <img 
              :src="getImageUrl(member.image)" 
              :alt="member.name"
              class="w-full h-full object-contain transition-transform duration-1000 group-hover:scale-105 origin-top" 
              @error="(e) => ((e.target as HTMLImageElement).src = getImageUrl('group.png'))"
            />
          </div>
          
          <!-- Solid Black Gradient Overlay for Maximum Text Readability -->
          <div class="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent opacity-90 transition-opacity duration-700 pointer-events-none"></div>
          
          <!-- Content Overlays -->
          <div class="absolute bottom-0 left-0 w-full p-6 md:p-12 flex flex-col justify-end h-full pointer-events-none">
            <div class="translate-y-8 opacity-70 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700 ease-out">
              <p class="text-xs md:text-sm font-black uppercase tracking-[0.3em] mb-2 md:mb-4 drop-shadow-md text-white/90">{{ member.role }}</p>
              <h2 class="text-2xl sm:text-4xl md:text-6xl lg:text-7xl font-black mb-2 md:mb-6 uppercase leading-[0.9] drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)]" :class="getGroupTheme(gIdx).name">{{ member.name }}</h2>
              
              <div class="flex gap-3 md:gap-4 pointer-events-auto">
                <a v-if="member.icons.includes('twitter')" href="#" class="w-10 h-10 md:w-14 md:h-14 rounded-full bg-[#0B0B0A]/80 text-white flex items-center justify-center backdrop-blur-md hover:bg-flax-smoke-500 hover:text-black transition-all duration-300 shadow-2xl border border-white/20">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 md:w-6 md:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z"></path></svg>
                </a>
                <a v-if="member.icons.includes('email')" href="#" class="w-10 h-10 md:w-14 md:h-14 rounded-full bg-[#0B0B0A]/80 text-white flex items-center justify-center backdrop-blur-md hover:bg-flax-smoke-500 hover:text-black transition-all duration-300 shadow-2xl border border-white/20">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 md:w-6 md:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                </a>
                <a v-if="member.icons.includes('linkedin')" href="#" class="w-10 h-10 md:w-14 md:h-14 rounded-full bg-[#0B0B0A]/80 text-white flex items-center justify-center backdrop-blur-md hover:bg-flax-smoke-500 hover:text-black transition-all duration-300 shadow-2xl border border-white/20">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 md:w-6 md:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg>
                </a>
                <a v-if="member.icons.includes('github')" href="#" class="w-10 h-10 md:w-14 md:h-14 rounded-full bg-[#0B0B0A]/80 text-white flex items-center justify-center backdrop-blur-md hover:bg-flax-smoke-500 hover:text-black transition-all duration-300 shadow-2xl border border-white/20">
...
<script setup lang="ts">
  ...
  const horizontalWrapper = ref<HTMLElement | null>(null);
  ...
  const initScrollAnimation = () => {
    ...
    const panels = gsap.utils.toArray('.horizontal-panel', horizontalWrapper.value);
    ...
    scrollTween = gsap.to(panels, {
      ...
      scrollTrigger: {
        ...
        pin: true,
        scrub: 1.5, // Butter smooth sync
        end: () => "+=" + (horizontalWrapper.value?.offsetWidth || 1000) * (panels.length * 0.5)
      }
    });
  };
  ...
  const goHome = () => {
    if (scrollTween && scrollTween.scrollTrigger) {
      if (lenis) {
        lenis.scrollTo(scrollTween.scrollTrigger.start, { duration: 1.5 });
      } else {
        window.scrollTo({ top: scrollTween.scrollTrigger.start, behavior: 'smooth' });
      }
    }
  };
  ...
  const goExit = () => {
    ...
    } else if (scrollTween && scrollTween.scrollTrigger) {
      if (lenis) {
        lenis.scrollTo(scrollTween.scrollTrigger.end, { duration: 1.5 });
      } else {
        window.scrollTo({ top: scrollTween.scrollTrigger.end, behavior: 'smooth' });
      }
    }
  };
  ...
  // Unified white theme for all groups
  const groupThemes = [
  ...
  const currentTeam = computed(() => {
    return allTeamsData[selectedYear.value] || [];
  });
  ...
  const groupedTeam = computed(() => {
    return roleGroups.map(group => {
      const members = currentTeam.value.filter(m => group.roles.includes(m.role));
...
/* Transitions for smooth year switching */
