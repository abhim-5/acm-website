<template>
  <div
    v-if="!isLoading"
    id="loading-screen"
    class="flex-center fixed bottom-0 z-99999 size-full cursor-wait bg-[#0e100f]"
  >
    <div class="size-full flex flex-col items-center justify-center">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 760 400" class="max-w-[1200px] w-full px-4">
        <defs>
          ...
          <linearGradient id="grad-2" x1="340" y1="42" x2="240" y2="125" gradientUnits="userSpaceOnUse">
            <stop offset="0.1" stop-color="#0ea5e9"></stop>
            <stop offset="0.5" stop-color="#ffffff"></stop>
          </linearGradient>
...
<script setup lang="ts">
  ...
  const triangleRef = ref<SVGPathElement>();
  const squareRef = ref<SVGPathElement>();
  const circleRef = ref<SVGPathElement>();
  ...
  onMounted(() => {
    ...
    // Create interpolators from the shapes to the ACM paths
    const interpA = interpolate(triangleRef.value!.getAttribute('d')!, pathA);
    const interpC = interpolate(squareRef.value!.getAttribute('d')!, pathC);
    const interpM = interpolate(circleRef.value!.getAttribute('d')!, pathM);
    ...
    morphTl.to(dummyA, {
      p: 1,
      onUpdate: () => triangleRef.value!.setAttribute('d', interpA(dummyA.p))
    }, 0)
    .to(dummyC, {
      p: 1,
      onUpdate: () => squareRef.value!.setAttribute('d', interpC(dummyC.p))
    }, 0.2)
    .to(dummyM, {
      p: 1,
      onUpdate: () => circleRef.value!.setAttribute('d', interpM(dummyM.p))
    }, 0.4);

    tl.add(morphTl);

    // Animate out the loading screen smoothly
